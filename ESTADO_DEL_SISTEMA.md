...
> coma**, **UTF-8 con BOM** (`EF BB BF`), fin de línea **CRLF**, nulos como cadena
> vacía, **todo el texto en mayúsculas** salvo las columnas de tipo no textual, y
> una columna **derivada** `documento_identidad` —nacionalidad + cédula, con relleno
> de ceros a la izquierda hasta 9 dígitos: `12345678` + `V` → `V012345678`— pegada a
> `cedula`. Son **62 columnas** en el archivo: las **61** de la vista más esa
> derivación. Las reglas viven en `csvDeExportacionHacer` y
> `FilaExportacionHacer.documentoIdentidad` (`lib/models/exportacion_hacer.dart`),
> **no** en la vista: la vista entrega datos, el formato lo pone el cliente.
